...
def load_config(config_path=CONFIG_PATH):
    try:
        ...
        if resp == "" or resp.upper() == "Y":
            cfg = generate_config()gitlab/master
            check = ""
        else:
            raise
...
def generate_backend_cfg_file(cfg):
    ...
    log_file = os.path.join(cfg['install_dir'], "opus.log")
    uds_sock = cfg['uds_path']
    db_path = cfg['db_path']
...
def start_opus_backend(cfg):
    ...
    try:
        ...
        if pid > 0:
            ...
            p_file.write(pid)
            p_file.close()
            os.execvp("python", ["python", run_server_path, backend_cfg_path])
    except OSError:
        sys.exit(1)
